...
static bool compare_pipeline(t_cmd *actual, t_cmd_test *expected, int expected_len)
{
    int i;
    int j;
    t_redir *r;
    ...
    while (actual && i < expected_len)
    {
        ...
        for (j = 0; j < expected[i].redir_count; j++)
        {
            ...
            if (r->type != expected[i].redirs[j].type)
                return (false);
            if (!r->redir_arg || strcmp(r->redir_arg, expected[i].redirs[j].redir_arg) != 0)
                return (false);
        ...
        if (cmds[i].redir_count)
        {
            ...
            for (j = 0; j < cmds[i].redir_count; j++)
                printf(j ? ", {%s \"%s\" q:%s}" : "{%s \"%s\" q:%s}",
                       rtype(cmds[i].redirs[j].type),
                       cmds[i].redirs[j].redir_arg,
...
static void print_actual(t_cmd *cmd)
{
    ...
    while (cmd)
    {
        ...
        if (r)
        {
            ...
            while (r)
            {
                printf(r != cmd->redir ? ", {%s \"%s\" q:%s}" : "{%s \"%s\" q:%s}",
                       rtype(r->type),
...
/* static void test_pipeline_cases(t_pipeline_test *tests, size_t n)
{
    ...
    for (i = 0; i < n; i++)
    {
        t_all *all = make_all(tests[i].exit_status);
        ...
        else
        {
            ...
            print_expected(tests[i].expected_cmds, tests[i].expected_cmd_count);
            printf(YELLOW "  Got:\n");
            if (result)
                print_actual(result);
            else
...
static void test_pipeline_cases(t_pipeline_test *tests, size_t n)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        else
        {
            ...
            if (tests[i].expect_syntax_fail)
            {
                printf(BLUE "  Expected: syntax_ret=<non-zero>, cmd=NULL\n" RESET);
                printf(YELLOW "  Got:      syntax_ret=%d, cmd=%s\n",
                       sr, result ? "(non-null)" : "NULL" RESET);
            }
            else
...
void test_pipeline(void)
{
    t_pipeline_test tests[] = {
        /* ── correct inputs ─────────────────────────────────────────────── */
        {
            ...
            .expected_cmds = (t_cmd_test[]){
                {"echo", (const char *[]){"echo", "hello", NULL}, NULL, 0},
            },
        ...
        },
        {
            ...
            .expect_syntax_fail = false,
            .expected_cmds = (t_cmd_test[]){
                {"cat", (const char *[]){"cat", NULL}, (t_redir_test[]){{REDIR_INPUT, "file", false}}, 1},
            },
            .expected_cmd_count = 1,
        },
        {
            ...
            .expect_syntax_fail = false,
            .expected_cmds = (t_cmd_test[]){
                {"echo", (const char *[]){"echo", "hi", NULL}, (t_redir_test[]){{REDIR_OUTPUT, "out", false}}, 1},
            },
            .expected_cmd_count = 1,
        },
        {
            ...
            .expect_syntax_fail = false,
            .expected_cmds = (t_cmd_test[]){
                {"echo", (const char *[]){"echo", "hi", NULL}, (t_redir_test[]){{REDIR_APPEND, "out", false}}, 1},
            },
            .expected_cmd_count = 1,
        },
        {
            ...
            .expect_syntax_fail = false,
            .expected_cmds = (t_cmd_test[]){
                {"cat", (const char *[]){"cat", NULL}, (t_redir_test[]){{REDIR_HEREDOC, "del", false}}, 1},
            },
            .expected_cmd_count = 1,
        },
        {
            ...
            .expect_syntax_fail = false,
            .expected_cmds = (t_cmd_test[]){
                {"cat", (const char *[]){"cat", "in", NULL}, (t_redir_test[]){
        ...
        },
        {
            ...
            .expected_cmds = (t_cmd_test[]){
                ...
                {"grep", (const char *[]){"grep", "foo", NULL}, NULL, 0},
            },
        ...
        },
        {
            ...
            .expect_syntax_fail = false,
            .expected_cmds = (t_cmd_test[]){
                {"echo", (const char *[]){"echo", "hello world", NULL}, NULL, 0},
            },
            .expected_cmd_count = 1,
        },
        {
            ...
            .expect_syntax_fail = false,
            .expected_cmds = (t_cmd_test[]){
                {"echo", (const char *[]){"echo", NULL}, (t_redir_test[]){{REDIR_OUTPUT, "my file", true}}, 1},
            },
            .expected_cmd_count = 1,
        },
        {
            ...
            .expect_syntax_fail = false,
            .expected_cmds = (t_cmd_test[]){
                {"grep", (const char *[]){"grep", "foo", "bar", NULL}, (t_redir_test[]){{REDIR_INPUT, "in", false}}, 1},
            },
            .expected_cmd_count = 1,
        },
        /* ── syntax errors ───────────────────────────────────────────────── */
        ...
        },
        {
            ...
            .expect_syntax_fail = false,
            .expected_cmds = (t_cmd_test[]){
                {"cat", (const char *[]){"cat", NULL}, (t_redir_test[]){{REDIR_INPUT, "", false}}, 1},
            },
            .expected_cmd_count = 1,
        },
        {
            ...
            .expect_syntax_fail = false,
            .expected_cmds = (t_cmd_test[]){
                {"cat", (const char *[]){"cat", NULL}, (t_redir_test[]){{REDIR_INPUT, "42", false}}, 1},
            },
            .expected_cmd_count = 1,
        },
        /* ── multiple pipes ──────────────────────────────────────────────── */
        ...
        },
        {
            ...
            .expect_syntax_fail = false,
            .expected_cmds = (t_cmd_test[]){
                {"cat", (const char *[]){"cat", NULL}, (t_redir_test[]){{REDIR_INPUT, "in", false}}, 1},
                {"grep", (const char *[]){"grep", "foo", NULL}, (t_redir_test[]){{REDIR_OUTPUT, "out", false}}, 1},
            },
            .expected_cmd_count = 2,
        },
        {
            ...
            .expect_syntax_fail = false,
            .expected_cmds = (t_cmd_test[]){
                {"cat", (const char *[]){"cat", NULL}, (t_redir_test[]){{REDIR_HEREDOC, "EOF", false}}, 1},
                {"sort", (const char *[]){"sort", NULL}, (t_redir_test[]){{REDIR_APPEND, "result", false}}, 1},
            },
            .expected_cmd_count = 2,
        },
        /* ── multiple redirections on one command ────────────────────────── */
        {
            ...
            .expect_syntax_fail = false,
            .expected_cmds = (t_cmd_test[]){
                {"cat", (const char *[]){"cat", NULL}, (t_redir_test[]){
        ...
        },
        {
            ...
            .expect_syntax_fail = false,
            .expected_cmds = (t_cmd_test[]){
                {"echo", (const char *[]){"echo", "hi", NULL}, (t_redir_test[]){
        ...
        /* ── many args ───────────────────────────────────────────────────── */
        {
            ...
            .expect_syntax_fail = false,
            .expected_cmds = (t_cmd_test[]){
                {"echo", (const char *[]){"echo", "a", "b", "c", "d", "e", NULL}, NULL, 0},
            },
            .expected_cmd_count = 1,
        },
        /* ── no-arg commands ─────────────────────────────────────────────── */
        ...
        /* ── $? expansion with two digits ────────────────────────────────── */
        {
            ...
            .expect_syntax_fail = false,
            .expected_cmds = (t_cmd_test[]){
                {"echo", (const char *[]){"echo", "127", NULL}, NULL, 0},
            },
            .expected_cmd_count = 1,
        },
        /* ── $? embedded inside a word ───────────────────────────────────── */
        {
            ...
            .expect_syntax_fail = false,
            .expected_cmds = (t_cmd_test[]){
                {"echo", (const char *[]){"echo", "exit1code", NULL}, NULL, 0},
            },
            .expected_cmd_count = 1,
        },
        /* ── more syntax errors ──────────────────────────────────────────── */
        ...
        /* ── quoting: adjacent quoted+unquoted segments merge into one arg ── */
        {
            ...
            .expect_syntax_fail = false,
            .expected_cmds = (t_cmd_test[]){
                {"echo", (const char *[]){"echo", "helloworld", NULL}, NULL, 0},
            },
            .expected_cmd_count = 1,
        },
        {
        ...
        /* ── quoting: single quotes suppress all expansion ───────────────── */
        {
            ...
            .expect_syntax_fail = false,
            .expected_cmds = (t_cmd_test[]){
                {"echo", (const char *[]){"echo", "$HOME", NULL}, NULL, 0},
            },
            .expected_cmd_count = 1,
        },
        {
        ...
        /* ── quoting: quoted heredoc delimiter suppresses expansion ──────── */
        {
            ...
            .expect_syntax_fail = false,
            .expected_cmds = (t_cmd_test[]){
                {"cat", (const char *[]){"cat", NULL}, (t_redir_test[]){{REDIR_HEREDOC, "EOF", true}}, 1},
            },
            .expected_cmd_count = 1,
        },
        {
